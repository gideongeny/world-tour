...
def cache_result(timeout=300):
    """Decorator to cache function results (disabled for development)"""
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            # Skip caching for now - just execute the function
    ...
    with app.app_context():
        ...
        if OfflineMap.query.count() == 0:
            ...
            maps = [
                OfflineMap(
                    destination_id=paris_dest.id if paris_dest else 1,
                    map_name='Paris City Map',
                    file_url='/static/maps/paris_offline.pdf',
                    file_size=2048576,  # 2MB
                    version='1.0'
                ),
                OfflineMap(
                    destination_id=tokyo_dest.id if tokyo_dest else 2,
                    map_name='Tokyo Metro Map',
...
# Add these new models after the existing models (around line 2219)
...
def sitemap():
    destinations = Destination.query.all()
    pages = ContentPage.query.filter_by(is_published=True).all()
